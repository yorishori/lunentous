...
// keystore.properties is gitignored (personal signing key, never
// committed) -- release signing is only configured when it's present, so
// a checkout without it still builds debug fine (e.g. CI, another dev).
...
android {
    namespace = "com.lunentous.app"
    compileSdk = 35

    defaultConfig {
        applicationId = "com.lunentous.app"
        minSdk = 26
        targetSdk = 35
        versionCode = 1
        versionName = "0.1.0"
...
dependencies {
    // Compose, via the BOM so individual artifact versions stay in sync.
